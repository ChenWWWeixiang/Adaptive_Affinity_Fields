...
IMG_MEAN = np.array((122.675, 122.669, 122.008), dtype=np.float32)
...
def get_arguments():
  ...
  parser.add_argument('--data-dir', type=str, default='/home/data1/U-net/H-DenseUNet/data/',
  ...
  parser.add_argument('--input-size', type=str, default='240,240',
  ...
  parser.add_argument('--kld-margin', type=float,default=3.0,
                      help='margin for affinity loss')
  parser.add_argument('--kld-lambda-1', type=float,default=1.0,
                      help='Lambda for affinity loss at edge.')
  parser.add_argument('--kld-lambda-2', type=float,default=1.0,
...
def main():
  ...
  input_size = (h, w)
  innet_size = (int(math.ceil(h/1)), int(math.ceil(w/1)))
  #innet_size=input_size
  ...
  image_batch, label_batch = tf.placeholder(tf.float32,shape=(args.batch_size,input_size[0],input_size[1],3),name='xin'),\
                             tf.placeholder(tf.int32,(args.batch_size,input_size[0],input_size[1],1),'yin')
  ...
  for i,output in enumerate(outputs):
    ...
    #seg_loss=seg_loss*tf.Tensor([0.78, 0.65,8.65])
    ...
    w_edge = tf.get_variable(
        name='edge_w',
        shape=(1,1,1,args.num_classes,1,4),
        dtype=tf.float32,
    ...
    eloss_3, neloss_3 = lossx.adaptive_affinity_loss(labels,
                                                     ...
                                                     w_edge[...,2],
                                                     w_not_edge[...,2])
    # Apply AAF on 9x9 patch.
    eloss_4, neloss_4 = lossx.adaptive_affinity_loss(labels,
                                                     one_hot_lab,
                                                     prob,
                                                     4,
                                                     args.num_classes,
    ...
    aaf_loss += tf.reduce_mean(eloss_4) * args.kld_lambda_1 * dec
    ...
    aaf_loss += tf.reduce_mean(neloss_4) * args.kld_lambda_2 * dec
  ...
  reduced_loss = mean_seg_loss + mean_l2_loss + mean_aaf_loss##try no aff
  ...
  for step in pbar:
    ...
    for it in range(args.iter_size):
      ...
      X, Y = reader.dequeue(args.batch_size)
      feed_dict = {step_ph: step,image_batch:X,label_batch:Y}
    ...
    if step%args.update_tb_every==args.update_tb_every/2 and step>0:
        X, Y = valreader.dequeue(args.batch_size)
        feed_dict = {step_ph: step, image_batch: X, label_batch: Y}
        summary = sess.run(img_summary,feed_dict=feed_dict)
        summary_writer.add_summary(summary, step)
